(function () {
  ...
  const Simulator = {
    ...
    setWidth(width, reason) {
      ...
      // Re-check padding after width change
      this.fixPadding();
    },
    setMode(mode) {
    ...
    toggleLive() {
      ...
      // Re-check padding after mode toggle to ensure view updates
      setTimeout(() => {
        this.fixPadding();
        // Trigger workspace refit if it exists
        if (window.Demeza.WorkspaceSystem && window.Demeza.WorkspaceSystem.fitSoon) {
          window.Demeza.WorkspaceSystem.fitSoon('toggleLive');
        }
      }, 50);
    },
    fixPadding() {
      // ENFORCEMENT LAYER:
      // Ensure bottom padding exactly matches left padding.
      const body = document.body;
      if (!body) return;

      const computed = window.getComputedStyle(body);
      const padLeft = computed.paddingLeft; // e.g., "24px"

      // Force padding-bottom to match padding-left
